...
map *load_map(char *filename) {
    ...
    for (int i = 0; i < m->height; i++) {
        ...
        for (int j = 0; j < m->width; j++) {
            ...
            if (m->level[i][j] != Wall)
                m->vert_nb++;
            else m->surface++; // un carré = 1m²

            if (m->level[i][j] == Start) {
                if (start_cnt >= m->start_nb) {
                    printf("Found too many starts, %d were specified", m->start_nb);
                    exit(0);
                }
            ...
            if (m->level[i][j] == Exit) {
                if (exit_cnt >= m->exit_nb) {
                    printf("Found too many exits, %d were specified", m->exit_nb);
                    exit(0);
                }
...
/*
 * Return neighbours of position (y,x) for adj. matrix
 * */
{
    /*
    0    1    2
    ...
    */
    ...
    if (DIAG == 1) {
        ...
        adj[2] = (y != 0 && x != m->width - 1 && m->level[y - 1][x + 1] != Wall) ? 1 : 0;
        adj[3] = (x != 0 && m->level[y][x - 1] != Wall) ? 1 : 0;
        adj[4] = (x != m->width - 1 && m->level[y][x + 1] != Wall) ? 1 : 0;
        adj[5] = (x != 0 && y != m->height - 1 && m->level[y + 1][x - 1] != Wall) ? 1 : 0;
        adj[6] = (y != m->height - 1 && m->level[y + 1][x] != Wall) ? 1 : 0;
        adj[7] = (x != m->width - 1 && y != m->height - 1 && m->level[y + 1][x + 1] != Wall) ? 1 : 0;
